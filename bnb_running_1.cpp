...
void branch_and_bound_tsp(const vector<vector<double>>& adj, vector<bool>& visited, int node, int depth, double cost, double& min_cost, vector<int>& best_tour, vector<int>& current_tour, unordered_map<vector<bool>, double>& mst_cache, list<vector<bool>>& cache_order, int cache_size, chrono::steady_clock::time_point endTime) {
    ...
    for (int i = 0; i < N; ++i) {
        if (!visited[i]) {
            ...
            branch_and_bound_tsp(adj, visited, i, depth + 1, cost + adj[node][i], min_cost, best_tour, current_tour, mst_cache, cache_order, cache_size, endTime);
...
int main(int argc, char* argv[]) {
    ...
    branch_and_bound_tsp(adj, visited, 0, 1, 0, min_cost, best_tour, current_tour, mst_cache, cache_order, cache_size, endTime);
    ...
    // Write min_cost and duration to a CSV file
    ofstream outputFile("results.csv", ios::app); // Open the file in append mode
    outputFile << fname << "," << min_cost << "," << duration << endl;
    outputFile.close();
