...
void TSPRec(const vector<vector<double>>& adj, vector<int>& currentPath, vector<bool>& visited, int currentLevel, double& minCost, vector<int>& bestPath, double currentCost) {
    int N = adj.size();
    ...
    for (int i = 0; i < N; ++i) {
        if (!visited[i]) {
            ...
            if (newCost + calculateMSTHeuristic(adj, visited) < minCost) {
                TSPRec(adj, currentPath, visited, currentLevel + 1, minCost, bestPath, newCost);
            }
...
void ParallelTSP(vector<vector<double>>& adj, int startNode, double& globalMinCost, vector<int>& globalBestPath, int N) {
    ...
    for (int nextNode = 0; nextNode < N; ++nextNode) {
        ...
        threads.push_back(thread([&, nextNode]() {
            ...
            TSPRec(adj, localPath, localVisited, 2, localMinCost, localBestPath, newCost);
...
int main() {
    ...
    for (int startNode = 0; startNode < N; ++startNode) {
        threads.push_back(thread(ParallelTSP, ref(adj), startNode, ref(globalMinCost), ref(globalBestPath), N));
    }
